...
def main() -> None:
    """Run corruption, repair, and three-state comparison."""
    settings = load_settings()

    if not settings.paths.baseline_metrics.exists() or not settings.paths.clean_json.exists() or not settings.paths.eval_testset.exists():
        run_phase1()

    baseline_metrics = read_json(settings.paths.baseline_metrics)
    clean_df = build_clean_dataframe(load_raw_records(settings.paths.raw_records_json), now_utc())
    ...
    for metric in ["retrieval_hit_rate", "mean_token_f1", "judge_accuracy", "mean_judge_score"]:
        print(
            f"| {metric} | "
            f"{float(baseline_metrics[metric]):.3f} | "
            f"{float(corrupted_bundle.summary[metric]):.3f} | "
            f"{float(repaired_bundle.summary[metric]):.3f} |"
        )
